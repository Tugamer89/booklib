...
export default {
    ...
    setup(props, { emit }) {
        ...
        return {
            coverPreview, isScanning,
            handleFileChange, startIsbnScanner, openGoogleSearch, onIsbnInput
        };
    },
    template: `
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            <label for="title" class="block text-sm font-medium text-slate-600 dark:text-slate-300">Titolo *</label>
                            <input v-model="bookData.title" type="text" name="title" id="title" required class="mt-1 input-style">
                        </div>
                        <div>
                            <label for="author" class="block text-sm font-medium text-slate-600 dark:text-slate-300">Autore *</label>
                            <input v-model="bookData.author" type="text" name="author" id="author" required class="mt-1 input-style">
                        </div>
                        <div>
                            <label for="publisher" class="block text-sm font-medium text-slate-600 dark:text-slate-300">Casa Editrice</label>
                        ...
                        <div>
                            <label for="location" class="block text-sm font-medium text-slate-600 dark:text-slate-300">Posizione *</label>
                            <input v-model="bookData.location" type="text" name="location" id="location" required class="mt-1 input-style" pattern="[A-Z]+[0-9]+" title="Formato non valido. Es: A5, LIBRERIA10" oninput="this.value = this.value.toUpperCase()">
                        </div>
                        <div>
                            <label for="language" class="block text-sm font-medium text-slate-600 dark:text-slate-300">Lingua (es. IT)</label>
                            <input v-model="bookData.language" type="text" name="language" id="language" maxlength="3" class="mt-1 input-style">
                        </div>
                        <div class="sm:col-span-2">
                            <label for="description" class="block text-sm font-medium text-slate-600 dark:text-slate-300">Descrizione</label>
                            <textarea v-model="bookData.description" name="description" id="description" rows="3" class="mt-1 input-style resize-none"></textarea>
                        </div>
                        <div class="sm:col-span-2">
                            <label for="personal_comment" class="block text-sm font-medium text-slate-600 dark:text-slate-300">Commento Personale</label>
                    ...
                    <div class="space-y-2">
                            <label class="block text-sm font-medium text-slate-600 dark:text-slate-300">Anteprima Copertina</label>
