...
def _send_email_brevo(to_email: str, subject: str, html_content: str) -> bool:
    if not settings.BREVO_API_KEY or not settings.BREVO_EMAIL_FROM_ADDRESS:
        logger.error(
            "BREVO_API_KEY o BREVO_EMAIL_FROM_ADDRESS non configurati. Impossibile inviare email."
        )
        return False
    ...
    try:
        ...
        logger.info(
            f"Email inviata a {to_email} via Brevo. Oggetto: {subject}. Response: {api_response.message_id if hasattr(api_response, 'message_id') else 'N/A'}"
        )
        return True
    except ApiException as e:
        error_body = e.body if hasattr(e, "body") else str(e)
        logger.error(
            f"Errore API Brevo inviando a {to_email}. Status: {e.status if hasattr(e, 'status') else 'N/A'}. Body: {error_body}"
        )
        return False
